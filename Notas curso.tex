...
\usepackage{caption}   % Rename captions, I dont know why I need a package for this
...
% TODO centrado: "¿Quieres la verdad? tú no puedes manejar la verdad..."
...
Expresiones como ``¿Qué hora es?'', ``¡Qué calor!'' o ``Haz tu tarea'' no son enunciados, ya que no son expresiones declarativas.
...
\begin{df}
	...
	que a cada enunciado, o pareja de ellos, les asigna alguna otra.
\end{df}
...
\begin{description}
	...
	Por ejemplo, si $p$ representa ``$2+2=5$", $\neg p$ es el enunciado que afirma ``no es cierto que $2+2=2$", o bien ``$2+2\neq 5$".
...
De aquí en adelante mantendremos el simbolismo de manera correcta, no debemos asignar interpretaciones, excepto tal vez en ejemplos, a los enunciados que aparezcan; de esta manera vamos a obtener generalidad en los resultados, podremos aislar el razonamiento lógico del semántico; y más importante, nos deshacemos del ruido que aparece con la complejidad de lo concreto.
...
En el párrafo anterior, nos referimos como enunciado atómico a los que no se puede descomponer propiamente en términos de otros enunciados y de los operadores lógicos, y diremos que un enunciado es \emph{compuesto} en caso contrario.
Por ejemplo, el enunciado $r \equiv \neg p \veebar q$ es compuesto, mientras que no necesariamente es el caso de $p,q$, que usualmente se asumen, cuando no explícitamente sean compuestas, como atómicas.

Una tabla de verdad de un enunciado $p$ es una guía que nos permite conocer el comportamiento de enunciados arbitrarios usando fuerza bruta.
Se analizan todas las combinaciones de los valores de verdad de todas sus enunciados atómicos que aparezcan en $p$, usualmente representados en una tabla.\footnote{también se suelen representar en diagramas de Venn, pero los evitaremos en este capítulo.}
\begin{ejm}
	...
	Se suele hacer, con el fin de evitar errores, usar una columna por cada \emph{sub-enunciado} implicado, por lo que la tabla anterior debería quedar así
...
Al momento de evaluar expresiones, uno tiene que decidir el orden de evaluación, por ejemplo si consideramos un enunciado $s=p\rightarrow q \vee r$, ¿cómo se evalúa? bien podría ser $(p\rightarrow q) \vee r$ o bien $p\rightarrow (q \vee r)$ por lo que nuestra expresión original $s$ podría parecer redundante. Una forma de arreglar este problema es forzando agregar un par de paréntesis por cada operación dentro de un enunciado compuesto; sin embargo, esto nos producirá enunciados cuya cantidad de símbolos crece muy rápidamente, haciéndolo muy difícil entender.
...
\begin{ejm}
	Si quisiéramos colocar explícitamente los paréntesis en un enunciado compuesto como \[p \rightarrow (q \vee r \veebar q \wedge (p \vee r)) \]
...
Como se puede ver el álgebra de operadores de un enunciado arbitrario queda totalmente determinada por su tabla de verdad, dentro de ella hay dos casos extremos que nos dan cierto interés: cuando siempre es falsa y cuando siempre es verdadera. Esto nos motiva a dar la siguiente definición.

\begin{df}
	Si $p$ es un enunciado, diremos que $p$ es una \emph{tautología} si su columna en una tabla de verdad está compuesto únicamente de valores $VERDADERO$.
	Y si por el contrario, si $p$ tiene una tabla de verdad que consiste únicamente de valores $FALSO$, se dice que $p$ es una \emph{contradicción}.
\end{df}
...
¡No hay de qué preocuparse! Cuando definamos, un poco más abajo en la siguiente sección, la igualdad entre enunciados (se llaman equivalencias lógicas) será claro que la tautología y la contradicción son únicas. % TODO: hacer justamente esto
...
\begin{ejm}
	Considere el enunciado $\psi \equiv \top \vee (q \veebar p)$. Uno puede mostrar que es una tautología haciendo una tabla de verdad como la siguiente.
	\begin{center}\begin{tabular}{|c|c||c|c|}
		\hline
...
Esto nos da una motivación para considerar la siguiente noción de ``igualdad'' entre enunciados.
...
Construyamos un ejemplo sobre lo que queremos hacer: la ley de la doble negación establece que $\neg \neg p \iff p$ sin tener interpretación sobre el enunciado $p$.
Cuando definimos una tabla de verdad, tuvimos el cuidado de analizar cada uno de las combinaciones que se pueden formar vía sus enunciados atómicos, y definimos equivalencia para que en cada una de las combinaciones los valores de verdad, en este caso de $\neg \neg p$ y $p$ coincidan.
Entonces, si pierdo generalidad, cambiando la interpretación de $p$ por otro enunciado, digamos $r \vee s$, la equivalencia debe mantenerse, y obtenemos la siguiente equivalencia: \[\neg \neg (r \vee s) \iff r \vee s. \]
...
\begin{thm}[Regla de sustitución 1]
	Si $\psi$ es una tautología, y $p$ un sub-enunciado atómico de $\psi$. Defina $\psi^\prime$ como el resultado de reemplazar en $\psi$ cada aparición de $p$ por un mismo enunciado (no necesariamente atómica) $q$. Entonces $\psi^\prime$ es también una tautología.
\end{thm}
...
La regla de sustitución nos dice que podemos sustituir cada aparición de un enunciado atómico por el enunciado que nos dé la gana, y aún así seguir obteniendo una tautología.
Veamos cómo podemos usar la regla de sustitución en un par de ejemplos más:
\begin{ejm}
	Considere la regla de De Morgan de la tabla \ref{tb_EquivalenciasLogicas} que dice $\neg(p \vee q) \iff \neg p \wedge \neg q$. Sustituya el enunciado atómico $p$ por $p\wedge \neg q$ y obtenemos una nueva equivalencia \[\neg ((p \wedge \neg q) \vee q) \iff \neg (p \wedge \neg q) \wedge \neg q\]
\end{ejm}
...
\begin{ejm}
	...
	\begin{align*}
		...
		\\ & \iff p & \text{Identidad}
	\end{align*}
	por lo tanto el enunciado $\psi$ original es equivalente simplemente a $p$. No es difícil convencerse de que es cierto usando una tabla de verdad.
\end{ejm}
\begin{ejm}
...
Sea $p$ el enunciado ``Juan repasa sus apuntes de matemáticas" y $q$ que sea ``Alguien invita a Juan a jugar".
Hay dos enunciados que nos da la situación, que lo podemos escribir simbólicamente: $\neg q\rightarrow p$ y también $\neg q$.
Recordemos que el propósito del operador condicional es determinar cuándo es verdad que si ocurren algunos enunciados (condiciones) se cumple algún otro enunciado (conclusión) que es justamente lo que queremos, de hecho queremos que eso suceda siempre, y por supuesto, que suceda siempre se llama tautología.
...
\begin{df}
	Si $\psi$ y $\phi$ son dos enunciados, diremos que $\psi$ implica $\phi$, denotado por $\psi \implies \phi$ cuando el enunciado $\psi \rightarrow \phi$ sea una tautología. 
\end{df}
...
\end{equation}
Si $p$ y $q$ son equivalentes entonces el lado izquierdo de la equivalencia \ref{eq_reduccion} es una tautología, por lo que la conjunción del lado derecho también lo es, y la única forma de que esto suceda es que tanto $p \rightarrow q$ y $q\rightarrow q$ sean ambas tautologías; esto es, que $p\implies q$ y $q \implies p$.
...
\begin{thm}\label{thm_equivalenciaEquivalencias}
	Dados dos enunciados $p,q$, son equivalentes
	\begin{enumerate}
...
Lo segundo: tenemos una nueva forma de obtener equivalencias: si queremos mostrar que $p \iff q$, basta probar que $p$ implica $q$ y que $q$ implica $p$; lo cual típicamente es más fácil que hacer una propia equivalencia usando tablas de verdad, o usando las leyes lógicas. % TODO: incluir demostraciones de las leyes lógicas
...
\begin{description}
	\item[Modus Ponens (MP)] % TODO: terminar ésta regla, y el resto. Deben incluir nombre, símbología y tabular, intuición, demostración y ejemplo de uso. 
...
\begin{ejm}
	...
	\begin{align*}
		p &\equiv \text{Ana hace pastel}\\
		q &\equiv \text{Ana practica su guitarra}\\
		r &\equiv \text{El padre de Ana le presta su auto a Ana.}
	\end{align*}
	% TODO: ¿ya habíamos mencionado antes qué es una premisa, o lo decimos aquí?
	entonces tenemos tres premisas: $p$, $p \rightarrow \neg q$, $\neg q \rightarrow \neg r$. Deberíamos poder concluir que no le prestó el carro, $\neg r$; puesto que
	\begin{enumerate}
		\item Como $p \rightarrow \neg q$ y $\neg q \rightarrow \neg r$ son premisas, usando la ley del silogismo podemos deducir que $p \rightarrow \neg r$.
...
Hagamos algunos ejemplos variados.

\begin{ejm}
	Para demostrar lógicamente que 
\begin{equation*}\begin{split}
		p \rightarrow r \\ 
		r \rightarrow s \\ 
		t \vee \neg s \\
		...
		\hline \therefore \neg p
\end{split}	\end{equation*}
podemos proceder como en los ejemplos pasados
	\begin{align*}
		\# \, & \text{enunciado} & \text{razón}\\
		1.& p \rightarrow r & \text{premisa} \\
		2.& r \rightarrow s & \text{premisa}\\
		3.& p \rightarrow s & \text{Sil(1,2)}\\
		4.& \neg t \vee u & \text{premisa}\\
		5.& t \rightarrow u & \text{equivalencia(4)}\\
		6.& \neg u & \text{premisa}\\
		7.& \neg t & \text{MT(6,5)}\\
		8.& t \vee \neg s & \text{premisa}\\
		9.& \neg t \rightarrow \neg s & \text{equivalencia(8)}\\
		10.& \neg s & \text{MP(7,9)}\\
		11. & \neg p & \text{MT(3,9)}
	\end{align*}
\end{ejm}
...
\begin{ejm}
	...
	\begin{equation*}\begin{split}
			...
			\hline \therefore \neg p
	\end{split}	\end{equation*}
	siguiendo la siguiente demostración
	
	\begin{align*}
	\# \, & \text{enunciado} & \text{razón}\\
	1.& \neg r \wedge (q \vee \neg q) & \text{premisa}\\
	2.& \neg r & \text{equivalencia(1)}\\
	3.& \neg r \rightarrow \neg q &\text{premisa}\\
	4.& \neg q & \text{MP(2,3)}\\
	5.& p \rightarrow q & \text{premisa}\\
	6.& \neg p & \text{MP(5,4)}	
\end{align*}
\end{ejm}

Las otras leyes lógicas que aparecen en la tabla \ref{sc_inferenciasLogicas} también pueden y van a ser usadas de aquí en adelante.

\begin{ejm}
	Demuestre que si se cumplen \begin{enumerate}
		\item $p \rightarrow q$
		\item $q\rightarrow r\wedge s$
		\item $\neg r \vee \neg t \vee u$
		\item $p\wedge t$
	\end{enumerate} 
	se puede deducir que $u$; para ello hacemos una demostración formal
	\begin{align*}
		\# \, & \text{enunciado} & \text{razón}\\
		1.& \neg r \vee \neg t \vee u & \text{premisa}\\
		2.& r \rightarrow \neg t \vee u & \text{equivalencia(1)}\\
		3.& p \wedge t & \text{premisa}\\
		4.& p & \text{SC(3)}\\
		5.& p \rightarrow q & \text{premisa}\\
		6.& q & \text{MP(4,5)}\\
		7.& q\rightarrow r\wedge s & \text{premisa}\\
		8.& r \wedge s & \text{MP(6,7)}\\
		9.& r & \text{SC(8)}\\
		10.&\neg t \vee u & \text{MP(9,2)}\\
		11.&t \rightarrow u & \text{equivalente(10)}\\
		12.&t & \text{SC(3)}\\
		13.&u & \text{MP(12,11)}
	\end{align*}
\end{ejm}
...
\begin{ejm}
	...
	Si la banda de rock no pudo tocar o los refrigerios no llegaron a tiempo, entonces se cancela la fiesta de año nuevo y Alicia se molesta.
	Si la fiesta se cancela, se tendría que dar reembolso; pero no se dieron reembolsos.
	
	Uno puede deducir que la banda sí tocó; para ello primero extraigámosle la forma al problema
	\begin{align*}
		p &\equiv \text{``la banda tocó''}\\
		q &\equiv \text{``los refrigerios llegaron a tiempo''}\\
		r &\equiv \text{``la fiesta se canceló''}\\
		s &\equiv \text{``Alicia se molesta''}\\
		t &\equiv \text{``hay reembolsos''}
	\end{align*}
	lo que nos da las siguientes premisas
	\begin{align*}
		\varphi_1 &\equiv (\neg p \vee \neg q)\rightarrow r \wedge s\\
		\varphi_2 &\equiv r \rightarrow t \\
		\varphi_3 &\equiv \neg t
	\end{align*}
	el lector podrá fácilmente demostrar que $\varphi_1 \wedge \varphi_2 \wedge \varphi_3 \implies p$.
\end{ejm}

Agreguemos un esquema de regla. Es común tener que demostrar una expresión condicional $p \rightarrow q$, en el que técnicamente pueda ser más difícil que probar $q$ mismo, entonces para facilitar la prueba o para darle claridad, se acostumbra argumentar a través de suposiciones. Es lógicamente correcto suponer que $p$ es verdadero, y usándolo como premisa extra, demostrar $p \rightarrow q$, o equivalentemente $q$.

% TODO: esquema de prueba por suposición

\begin{table}[h] %TODO ¿en página completa?
	\caption{Lista de leyes de inferencia con sus nombres largos y cortos}
	\label{tb_TablaInferencias}
	\centering
\begin{tabular}{|c|c|}
	\hline
	Regla de inferencia & Nombre de la regla \\
	\hline
	$\begin{aligned}
		p \\
		p \rightarrow q \\ 
		\hline \therefore q
	\end{aligned}$ & Modus ponens(MP) \\
	\hline
	$\begin{aligned} 
		p \rightarrow q \\
		q \rightarrow r \\
		\hline \therefore p \rightarrow r
	\end{aligned}$
	&  Ley del silogismo (Sil) \\
	\hline
	$\begin{aligned} 
		p \rightarrow q \\ \neg q \\ \hline \therefore \neg p
	\end{aligned}$
	...
	\hline
	$\begin{aligned} 
		p \\ q \\ \hline \therefore p \wedge q
	\end{aligned}$
	&  Regla de conjunción (Con) \\
	\hline
	$\begin{aligned} 
	...
	\hline
	$\begin{aligned} 
		\neg p \rightarrow \bot \\ \hline \therefore p
	\end{aligned}$
	& Regla de contradicción (!) \\
	\hline
	$\begin{aligned} 
		p\wedge q \\\hline  \therefore p
	\end{aligned}$
	& Regla de simplificación conjuntiva (SC)  \\
	\hline
	$\begin{aligned} 
		p \\ \hline \therefore p \vee q
	\end{aligned}$
	& Regla de amplificación disyuntiva (AD)  \\
	\hline
	$\begin{aligned} 
		p \wedge q \\ p \rightarrow (q \rightarrow r) \\ \hline \therefore r
	\end{aligned}$
	& Regla de la prueba condicional (PC)  \\
	\hline
	$\begin{aligned} 
		p \rightarrow r \\ q \rightarrow r \\ \hline \therefore (p\vee q) \rightarrow r 
	\end{aligned}$
	& Regla de la prueba por casos (Cas) \\
	\hline
	$\begin{aligned} 
		p \rightarrow q \\ r \rightarrow s \\ p \vee r \\ \hline \therefore q \vee s
	\end{aligned}$
...
% TODO inferencias sobre enunciados cuantificados
...
\begin{df}
	Un \emph{enunciado abierto} es una expresión declarativa con términos no declarados (variables libres) de tal manera que al sustituirlos por objetos de algún tipo específico se convierte en enunciado.
\end{df}
...
\begin{ejm}
	...
	Si se sustituye $x$ por cualquier objeto (número en este contexto) se convierte en un enunciado, algo así como \[p(\pi) \equiv \cos^2(\pi) + \sin^2(\pi) = 1\]
	que uno se puede convencer que es un enunciado, que es verdadero.
	
	Nótese que el enunciado abierto $p(x)$ no tiene asignado un valor de verdad ya que no es un enunciado: la expresión \[\cos^2 x + \sin^2 x = 1\]
...
\[q \equiv \cos^2 x + \sin^2 x = 1,\quad \text{para cualquier } x \]
...
En algunos casos se puede notar que de manera canónica, un conjunto es más pequeño que otro como por ejemplo $\set{0}$ es más pequeño que $\set{0,1}$ y este último más pequeño que $\set{0,1,2}$, pero por otro lado $\set{0}$ no es más pequeño, en este sentido que $\set{1,2}$ ya que, a pesar de que el primero tenga menos elementos que el segundo, $0$ no forma parte de $\set{1,2}$.
Formalicemos esta noción de \emph{ser más pequeño} en la siguiente definición.
\begin{df}
